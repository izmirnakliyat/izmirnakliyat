...
if (!function_exists('mynak_dashboard_seo_metrics_column_exists')) {
    function mynak_dashboard_seo_metrics_column_exists(\mysqli $conn, string $table, string $column): bool
    {
...
if (!function_exists('mynak_dashboard_seo_metrics_scan_table')) {
    /**
     * @param \mysqli $conn
     * @param array{table:string,title_col:string,meta_col?:string,title_fallback_col?:string,status_col:string,status_val:int} $cfg
    ...
    function mynak_dashboard_seo_metrics_scan_table(\mysqli $conn, array $cfg): array
    {
        ...
        $metaCol = preg_replace('/[^a-z0-9_]/i', '', $cfg['meta_col'] ?? 'meta_description');
        $fallbackCol = preg_replace('/[^a-z0-9_]/i', '', (string) ($cfg['title_fallback_col'] ?? ''));
        $statusCol = preg_replace('/[^a-z0-9_]/i', '', $cfg['status_col']);
        $statusVal = (int) $cfg['status_val'];

        if ($fallbackCol !== '' && mynak_dashboard_seo_metrics_column_exists($conn, $table, $fallbackCol)) {
            $titleExpr = "COALESCE(NULLIF(TRIM(`{$titleCol}`), ''), `{$fallbackCol}`) AS t";
        } else {
            $titleExpr = "`{$titleCol}` AS t";
        }
        $scoreSelect = mynak_dashboard_seo_metrics_column_exists($conn, $table, 'seo_score')
            ? ', seo_score AS s'
            : '';
        $sql = "SELECT {$titleExpr}, `{$metaCol}` AS m{$scoreSelect} FROM `{$table}` WHERE `{$statusCol}` = {$statusVal}";
        $res = @$conn->query($sql);
        if (!$res) {
            error_log('MyNakliyat dashboard_seo_metrics scan failed (' . $table . '): ' . $conn->error);
            return $bucket;
        }
        $scoreSum = 0;
        $scoreCount = 0;
