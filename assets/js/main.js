...
(function ($) {
    ...
    // Smooth Animation - Temporarily disabled for scroll issues
    ...
    $(document).ready(function () {
        ...
        var mainSlider;
        try {
            var slideCount = document.querySelectorAll('.main-slider .swiper-slide').length;
            var sliderOptions = {
                speed: 1500,
            ...
            function addAnimation() {
                scrollers.forEach((scroller) => {
                    ...
                    const scrollerContent = Array.from(scrollerInner.children).filter(
                        (item) => !item.classList.contains('scroller__clone')
                    );

                    scrollerContent.forEach((item) => {
                        const duplicatedItem = item.cloneNode(true);
                        duplicatedItem.setAttribute('aria-hidden', 'true');
                        duplicatedItem.classList.add('scroller__clone');
                        scrollerInner.appendChild(duplicatedItem);
                    });
                });
            }
        } catch(e) {
            console.warn('Scroller init error:', e);
        }
        ...
        //Gsap Text Animation
        var device_width = window.innerWidth || 0;
        try { textAnimation(); } catch(e) { console.warn('textAnimation error:', e); }
